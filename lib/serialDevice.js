...
module.exports = function(config) {
      ...
      async.eachSeries(ports, function(port, done) {
        ...
        chrome.serial.connect(port.path, connectopts, function(conn) {
          if (chrome.runtime.lastError !== undefined) {
            ...
            var errorText = chrome.runtime.lastError.message + 'Please try again.';
        ...
        if (result) {
          ...
          var deviceDebugInfo = 'driverId ' + deviceInfo.driverId + ', vendorId ' + deviceInfo.vendorId +
            ', productId ' + deviceInfo.productId + ', portPattern ' + deviceInfo.portPattern +
  ...
  function flush() {
    ...
    packetBuffer = [];
    if(connection) {
      chrome.serial.flush(connection.connectionId, function() { });
    }
  }
