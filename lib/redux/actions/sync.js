...
import { addInfoToError, getAppInitErrorMessage, getLoginErrorMessage, getLogoutErrorMessage, getUpdateProfileErrorMessage, UnsupportedError } from '../utils/errors';
...
export function setUserInfoFromToken(results) {
  const { user, profile, memberships, uploadHistory } = results;
  return {
    type: actionTypes.SET_USER_INFO_FROM_TOKEN,
    payload: { user, profile, memberships, uploadHistory },
...
export function uploadHistorySuccess(uploadHistory) {
  return {
    type: actionTypes.UPLOAD_HISTORY_SUCCESS,
